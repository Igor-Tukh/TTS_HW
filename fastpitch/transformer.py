...
class FFTransformer(nn.Module):
    def __init__(self, n_layer, n_head, d_model, d_head, d_inner, kernel_size,
        ...
        if embed_input:
            ...
            self.prom_emb = nn.Embedding(4, d_embed or d_model)
    ...
    def forward(self, dec_inp, seq_lens=None, conditioning=0, proms=None):
        ...
        proms_emb = 0 if proms is None or self.word_emb is None else self.prom_emb(proms) * mask

        out = self.drop(inp + pos_emb + conditioning + proms_emb)
